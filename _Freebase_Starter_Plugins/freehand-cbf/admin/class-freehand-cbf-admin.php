 ...
 * @package    Freehand_Cbf
 * @subpackage Freehand_Cbf/admin
 * @author     Freehand <sam@freehand.studio>
 */
class Freehand_Cbf_Admin {
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->freehand_cbf_options = get_option($this->plugin_name);
	}
	...
	public function enqueue_styles() {
		 ...
		 * defined in Freehand_Cbf_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Freehand_Cbf_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

		if ( 'settings_page_freehand-cbf' == get_current_screen() -> id ) {
             // CSS stylesheet for Color Picker
             wp_enqueue_style( 'wp-color-picker' );            
             wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/freehand-cbf-admin.css', array( 'wp-color-picker' ), $this->version, 'all' );
         }
	...
	public function enqueue_scripts() {
		...
		if ( 'settings_page_freehand-cbf' == get_current_screen() -> id ) {
		            wp_enqueue_media();   
		            wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/freehand-cbf-admin.js', array( 'jquery', 'wp-color-picker', 'media-upload' ), $this->version, false );         
		        }
	...
	public function add_plugin_admin_menu() {
	    ...
	    add_options_page( 'Freehand WP Cleanup and Base Options Functions Setup', 'Theme Setup', 'manage_options', $this->plugin_name, array($this, 'display_plugin_setup_page')
	    );
	}
	...
	public function display_plugin_setup_page() {
	    include_once( 'partials/freehand-cbf-admin-display.php' );
	}
	...
	public function validate($input) {
	        ...
	        $valid['footer_logo'] = (isset($input['footer_logo']) && !empty($input['footer_logo'])) ? 1 : 0;

	        $valid['options_page'] = (isset($input['options_page']) && !empty($input['options_page'])) ? 1 : 0;
	    ...
	    /**
	        ...
	        * @since    1.0.0
	        */
	        private function freehand_cbf_login_logo_css(){
	            if(isset($this->freehand_cbf_options['login_logo_id']) && !empty($this->freehand_cbf_options['login_logo_id'])){
	                $login_logo = wp_get_attachment_image_src($this->freehand_cbf_options['login_logo_id'], 'medium');
	        ...
	        private function freehand_cbf_login_background_color(){
	            if(isset($this->freehand_cbf_options['login_background_color']) && !empty($this->freehand_cbf_options['login_background_color']) ){
	                $background_color_css  = "body.login{ background:".$this->freehand_cbf_options['login_background_color']."!important;}";
	                return $background_color_css;
	            }
	        }
	        // Get Button and links color is set and different from #00A0D2 return it's css
	        private function freehand_cbf_login_button_color(){
	            if(isset($this->freehand_cbf_options['login_button_primary_color']) && !empty($this->freehand_cbf_options['login_button_primary_color']) ){
	                $button_color = $this->freehand_cbf_options['login_button_primary_color'];
	        ...
	        public function freehand_cbf_login_css(){
	            if( !empty($this->freehand_cbf_options['login_logo_id']) || $this->freehand_cbf_login_background_color() != null || $this->freehand_cbf_login_button_color() != null){
	                echo '<style>';
	                if( !empty($this->freehand_cbf_options['login_logo_id'])){
	                      echo $this->freehand_cbf_login_logo_css();
	                }
	                if($this->freehand_cbf_login_background_color() != null){
	                      echo $this->freehand_cbf_login_background_color();
	                }
	                if($this->freehand_cbf_login_button_color() != null){
	                      echo $this->freehand_cbf_login_button_color();
	                }
	                echo '</style>';
	            }
	        }
